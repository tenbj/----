...
> `new_project.ps1` 必须保持 UTF-8 with BOM。Trae Write/SearchReplace 会破坏 BOM，导致中文乱码。

修改 `new_project.ps1` 时，必须通过 ASCII-only 临时脚本或 RunCommand 调用 `[System.IO.File]::WriteAllText(path, content, [System.Text.UTF8Encoding]::new($true))` 写入。被破坏时通过读写 API 恢复 BOM。
