<#
.SYNOPSIS
    File, folder, config, and memory backup script.
.DESCRIPTION
    Supports four modes:
    - FILE   : output/ content files, backup to .history and rename the live file to a new version
    - FOLDER : .agent/skills/ folders, snapshot the whole folder to .history with a timestamp
    - CONFIG : .agent/rules/ files, copy to .history with a timestamp
    - MEMORY : versioned .memory artifacts, keep a stable live filename and write old versions to .history

    Memory design:
    - Append-only files under .memory/瀵硅瘽璁板綍 and .memory/绯荤粺璁板綍 never use this script
    - Versioned files under .memory/鐭ヨ瘑鎻愮偧 and .memory/鍏ㄥ眬鐭ヨ瘑鍦板浘.md use MEMORY mode
    - .memory folder snapshots are forbidden to avoid duplicate backup semantics
#>
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [ValidateSet("FILE", "FOLDER", "CONFIG", "MEMORY", "AUTO")]
    [string]$Mode = "AUTO"
)
...
function Is-MemoryPath {
    param([string]$Path)
    $normalized = Normalize-PathString $Path
    return $normalized -match '\\\.memory(\\|$)'
}
...
if ($Mode -eq "AUTO") {
    if ($item.PSIsContainer) {
        ...
        $Mode = "FOLDER"
    } elseif ($normalizedTarget -match '\\\.agent\\rules\\') {
        $Mode = "CONFIG"
...
try {
    ...
    Copy-Item -Path $TargetPath -Destination $newVersionFilePath -Force
    Write-Warning "Rename failed; created the new version by copying instead. The old file still exists and may need manual cleanup."
    Write-Host "[OK] Created new version by copy: $newVersionFilePath"
}
