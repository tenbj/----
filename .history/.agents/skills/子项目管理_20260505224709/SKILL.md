...
> 子项目内部固定分为 `01_问题答疑_v*`、`02_课题研究_v*`、`03_代码程序_v*` 三个子文件夹；三者的版本号只代表该分类的内容状态，空文件夹为 `v0.0.0`，有内容后为 `v1.0.0` 起步。
...
> 说明：Rule、Skill、脚本、备份、Memory 治理等系统对话，不写入该文件，统一由 `记忆管理` Skill 路由到 `.memory/系统记录/` 下的五个固定文件。
...
4. 按 `{编号}_{标题}.{扩展名}` 格式创建文件，内部文件不再带版本号，例如 `01_睡眠基础知识.md`
5. 同步更新 `目录.md` 对应分节，追加一行
6. 首次落盘日期与最近更新日期相同
...
│       ├── 目录.md                    ← 自动创建（文件索引）
│       ├── 版本记录.md                ← 自动创建（变更日志）
│       ├── 01_问题答疑_v0.0.0\        ← 空则 v0.0.0，有内容后 v1.0.0
│       │   └── 01_睡眠基础知识.md      ← 内部文件：编号 + 标题，不带版本号
│       ├── 02_课题研究_v1.0.0\
│       │   └── 01_褪黑素原理.md
│       └── 03_代码程序_v0.0.0\
│           └── 01_睡眠记录脚本.ps1
...
| `版本记录.md` | 固定文件 | 名称不变 | `版本控制备份` Skill 自动维护 |
| `01_问题答疑_v{x.y.z}/` | 分类文件夹 | 空为 `v0.0.0`，有内容后 `v1.0.0` 起步 | 存放问答型输出 |
| `02_课题研究_v{x.y.z}/` | 分类文件夹 | 空为 `v0.0.0`，有内容后 `v1.0.0` 起步 | 存放结构化研究输出 |
| `03_代码程序_v{x.y.z}/` | 分类文件夹 | 空为 `v0.0.0`，有内容后 `v1.0.0` 起步 | 存放代码、脚本、程序资产 |
| `{NN}_{标题}.{扩展名}` | 内部文件 | NN：两位编号，按所在分类内落盘顺序递增；不带版本号 | 对话产出 |

> `目录.md` 固定分为 `01_问题答疑`、`02_课题研究`、`03_代码程序` 三节，每节表格格式：`| # | 文件 | 说明 | 首次落盘 | 最近更新 |`
...
- ❌ 给内部内容文件追加 `_v*` 版本号（版本号由子项目文件夹和分类子文件夹承担）
