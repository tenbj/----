...
.DESCRIPTION
    ...
    3. Appends migration entry to version-log.md
...
$S_migration     = Get-Chinese (@(0xE8,0xBF,0x81,0xE7,0xA7,0xBB))
$S_migrated      = Get-Chinese (@(0xE5,0xB7,0xB2,0xE8,0xBF,0x81,0xE7,0xA7,0xBB))
...
if ($files.Count -eq 0) {
    Write-Host "No content files found to migrate."
    exit 0
}
...
# Append migration entry to version-log.md
$versionPath = Join-Path $ProjectPath $versionFile
if (Test-Path $versionPath) {
    $vc = [System.IO.File]::ReadAllText($versionPath)
} else {
    $vc = "# $S_versionLog $S_dot $topic`r`n`r`n> $S_migrated`r`n`r`n---`r`n`r`n"
}

$entry = "`r`n## $S_migration ($displayTimestamp)`r`n`r`n"
