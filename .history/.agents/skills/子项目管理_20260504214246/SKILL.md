...
```powershell
# 先预览（DryRun）
powershell -ExecutionPolicy Bypass -File "e:\my_project\知识研究\.agents\skills\子项目管理\scripts\migrate_project.ps1" -ProjectPath "output\00_系统治理_v1.1.4" -DryRun

# 确认无误后执行
powershell -ExecutionPolicy Bypass -File "e:\my_project\知识研究\.agents\skills\子项目管理\scripts\migrate_project.ps1" -ProjectPath "output\00_系统治理_v1.1.4"
```
...
> 本 Skill 下所有 `.ps1` 脚本（`new_project.ps1`、`next_number.ps1`、`migrate_project.ps1`）必须保持 UTF-8 with BOM。Trae Write/SearchReplace 会破坏 BOM，导致中文乱码。
