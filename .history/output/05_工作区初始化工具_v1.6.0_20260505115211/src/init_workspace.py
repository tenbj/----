...
再用 EXE 内置模板替换对应骨架资产；memory、input、output 只补缺。
...
def main():
    ...
    if agent_dir.exists() and any(agent_dir.iterdir()):
        ...
        answer = messagebox.askyesno(
            "目录已存在内容",
            f"目标目录已存在 .agents/ 内容：\n{workspace}\n\n是否继续升级骨架？\n\n工具会先备份受管 Rules/Skills，再替换为内置 v{SKELETON_VERSION} 模板；output、input、memory 只补缺。",
        )
        root.destroy()
        if not answer:
            return
