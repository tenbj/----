...
| 2 | .agents/ | 目录结构（rules/ + 8 个 skills/）+ 关键文件存在 | `02_.agents目录完整性` |
| 3 | output/ | 子项目文件夹命名 `{编号}_{主题}_v{版本}`，无零散文件；新结构子项目补查三类子文件夹、`目录.md`、`版本记录.md`、内部文件 `{NN}_` 编号和无 `_v*` 后缀 | `03_output结构命名规范` + 框架体检运行项 |
