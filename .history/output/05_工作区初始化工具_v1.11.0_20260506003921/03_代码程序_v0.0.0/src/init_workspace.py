...
SKELETON_VERSION = "1.9.0"
SSO_SPEC_VERSION = "1.1.0"
...
def _init_system_standards(workspace: Path, templates: Path):
    """初始化 .system/standards/ 下三个标准文件（仅补缺，已有则跳过）。"""
...
def initialize_workspace(workspace: Path, templates: Path) -> dict:
    ...
    # 1. 必须创建的空目录（对应 workspace-spec.json v1.1.0 requiredLayer.directories）
    skeleton_dirs = [
        ".agents/rules",
        ".agents/skills",
        ".history/output",
    ...
    # 5. 初始化 .system/standards/ 标准文件（仅当文件不存在时写入）
    _init_system_standards(workspace, templates)
