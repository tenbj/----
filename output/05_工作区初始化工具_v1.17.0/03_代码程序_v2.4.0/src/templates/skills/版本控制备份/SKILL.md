...
> ⚠️ **核心骨架授权门**：`.system/standards/`、`.agents/rules/`、`.agents/skills/`、`.agents/skills/*/scripts/`、05 初始化模板/源码/构建产物、`.memory/全局知识地图.md`、`.memory/知识提炼/*.md` 属于核心骨架或版本化核心文件。修改这些路径前必须先确认用户已明确授权。备份是必要条件，不等于授权；未授权时只能读取、分析、出方案和列清单。
...
> - `.system/standards` 的唯一合法快照位置是 `.history/.system/standards_{yyyyMMddHHmmss}/`。
...
```powershell
powershell -ExecutionPolicy Bypass -File ".agents\skills\版本控制备份\scripts\backup.ps1" `
